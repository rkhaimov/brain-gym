...
репозиториев, 5 стримов ~25 разработчиков.
...
* Сначала прямые импорты в app-shell были заменены динамическими (ремоуты из лок. пространства, фиксированные модули)
...
* По прежнему оставалась монорепа, однако стал доступен независимый деплой (но нам это не было нужно фактически)
* За пару месяцев до сдачи, разделил проект на раздельные репозитории. Подтягивал app-shell build tools как пакет
* Сам app-shell тянул модули из стенда (указывался разработчиком) + оверайдился локальным модулем + другими модулями
  собранными локально
* Core тянулся отдельным npm пакетом, сам лежал в отдельном репозитории
* Для деплоя нужно было собрать модуль, запушить артефакт на стенд
* capabilities API подтягивались как singleton empty пакет в runtime для использования механизма dep neg от webpack
