...
В ООП такая связь обычно реализуется через наследование:

```typescript
class Uniq extends Eq {
  unique(elements) {
    this.eq;
  }
}
...
Ввиду некоторых ограничений, наследование может заменяться композицией, что уже больше будет походить на функциональный
подход.
