...
Имеется три разные страницы, на которых используется один и тот же компонент - кнопка.

*Рис - диаграмма трех страниц и кнопки*
...
*Рис - подсветка страницы вызывающей требование*

Одним из вариантов, который может прийти в голову - это добавление нового аттрибута `guard` в компонент кнопки.

*Рис - указание у кнопки нового аттрибута*

Такое изменение, вызовет сложности следующего характера:

* Новый обязательный аттрибут нарушает обратную совместимость, что потребует дополнительных работ по интеграции
* Теперь, любые изменения во внешнем интерфейсе, связанные с необходимостью обеспечить работу `guard`, также повлекут за
  собой изменения в зависимых модулях
* Могут потребоваться дополнительные работы связанные с подменной сайд эффектов при тестировании. Для определения роли
  может отправляться запрос, либо считываться информация из внешнего хранилища.
...
*Рис - `RoleGuardButton` используемый только одной страницей*
...
*Рис - `Button` с опциональным свойством*
...
Каким образом, факт изменения компонента `Button` повлияет на хрупкость решения?
...
*Рис - `Button` с опциональным свойством желтого цвета*

Стоит понять, что был изменен исходный код компонента, т. е. его внутренняя реализация. Если говорить фактически, то
была изменена реализация *всех* зависимых от `Button` страниц.

*Рис - С подсветкой всех элементов желтым цветом*
...
*Рис - тестирование и реальное использование `Button`*
...
*Рис - страницы с желтыми элементами*
...
*Рис - страницы с желтыми элементами `createPlayer` с зависимостью `SinglePlayer`*
...
*Рис - страницы с элементами `createPlayer` с зависимостью `SinglePlayer`*
...
*Рис - кол стек страницы, `createPlayer` и многих других*
...
*Рис - с папками storage, actions, components pages*
...
увеличивает его *непрозрачность*.

#### Решение

Теперь взглянем на другой пример.

*Рис - с папками UserList, CreateUser, RemoveUser, Login*
...
*Рис - Reducer, Action, Selector, View*
...
Если предположить что решение является не избыточным, т. е. существуют *объективные* причины её обоснования, то тогда
...
*Рис - Проект из 3 микросервисов. Список пользователей, Форма пользователя и Платформа*

Далее предположим, что каждое изменение в них проходит через долгую процедуру проверок, тестов, сборки, версионирования
и развертывания.
...
Инкапсуляция - это намеренное сокрытие деталей реализации с целью *упрощения* её дальнейшего изменения. Дополнительным
эффектом также является и последующее упрощение внешнего интерфейса модуля.
...
*Рис - Две страницы (Создание пользователя и Создание админа) и свой компонент поиска у каждой*
...
*Рис - Две страницы с общим `SearchableSelect`*
...
*Рис - Две страницы с общим `SearchableSelect` (изменен селект)*
...
*Рис - Две страницы с общим `SearchableSelect` (изменено все)*
...
*Рис - Две страницы с каждым компонентом под себя*
...
*Рис - Две страницы и свой компонент поиска у каждой*
