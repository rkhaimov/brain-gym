...
const loadSkill = Tool.create({
  ...
  fn: (args, context): ToolMessage => {
    if (Either.isLeft(args)) {
      return args.value;
    }

    const skill = SKILLS.find((it) => it.name === args.value.skill);
    ...
    return {
      role: 'tool',
      name: context.meta.function.name,
      content: `Skill ${args.value.skill} not found. Available skills: ${SKILLS.map((it) => it.name).join(', ')}`,
    };
  },
});
...
export const Skills = {
  ...
  ${SKILLS.map((it) => `- **${it.name}**: ${it.description}`).join('\n')}
  
  Use the ${loadSkill.meta.function.name} tool when you need detailed information
